...
- **Local AWS** is `floci` (LocalStack-style emulator) on port 4566 with dummy credentials. Add local buckets/queues to `compose/floci/start.d/10-setup-resources.sh`, which runs at container start (it creates the source-docs, managed-docs and managed-doc-assets buckets, and enables versioning on managed-docs). `compose/mongo/10-init.js` seeds MongoDB (also a placeholder).
